...
// Parallel Processing - Dijkstra's Philosophers4 Problem - Spring 2019
// VERSION 4 -- Deadlock and starvation free with customizable number of philosphers
...
public class Philosophers4 extends Thread
{
	...
	Philosophers4(int left, int right, int id)
	{
		this.left = left;
		this.right = right;
		this.id = id;
	...
	public static void main(String[] args) throws Exception
	{
		if (args.length == 0 )
		{
			System.out.println("Proper Syntax: java Philosophers4 <number_of_philosophers");
			return;
		}

		// Take in number of chopsticks from the command line
		NUM_CHOPSTICKS = Integer.parseInt(args[0]);
		...
		for (int i = 0; i < NUM_CHOPSTICKS; i++)
		{
			...
			if (i < 4)
				philosphers[i] = new Thread(new Philosophers4(i % 5, (i + 1) % 5, i + 1));
			else
				philosphers[i] = new Thread(new Philosophers4((i + 1) % 5, i % 5, i));
